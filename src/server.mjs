  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          looksHtml
            ? `공공데이터 서버 오류(HTTP ${res.status}) — 잠시 후 다시 시도하세요. 계속되면 인증키·서비스 신청(승인) 상태를 확인하세요.`
      ...
      try {
        ...
        throw new Error(
          looksHtml
            ? '서버가 JSON 대신 오류 페이지를 반환했습니다 — 인증키 형식(Decoding)·서비스 신청 상태를 확인하세요.'
...
async function neisFetch(path, params) {
  const q = new URLSearchParams({ KEY: config.neis.key, Type: 'json', ...params }).toString();
